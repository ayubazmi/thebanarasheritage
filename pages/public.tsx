...
const HeroCarousel: React.FC<{ slides: any[] }> = ({ slides }) => {
    ...
    return (
        <div className="w-full bg-brand-50 pt-4 pb-6 md:pt-6 md:pb-8">
            <div className="container mx-auto px-4 md:px-6">
                <div className="relative h-[60vh] md:h-[80vh] w-full rounded-2xl md:rounded-3xl overflow-hidden shadow-sm group">
                        ...
                        >
                            ...
                            <div className="absolute inset-0 flex items-center justify-center text-center p-6">
                                <div className="max-w-2xl px-6 text-white animate-fade-in-up">
                                    {slide.title && <h1 className="text-4xl md:text-6xl font-serif font-bold mb-4 leading-tight drop-shadow-lg">
                                        {slide.title}
                                    </h1>}
                                    {slide.subtitle && <p className="text-base md:text-lg mb-8 font-light max-w-lg mx-auto drop-shadow-md opacity-90 block">
                                        {slide.subtitle}
                                    </p>}
                                    {slide.buttonText && (
                                        <Link to={slide.buttonLink || '/shop'}>
                                            <button className="bg-white text-brand-900 px-8 py-3 font-medium tracking-wide hover:bg-brand-50 transition-colors shadow-lg rounded-sm">
                                                {slide.buttonText}
                                            </button>
                                        </Link>
                                    )}
                    ...
                    {slides.length > 1 && (
                        <>
                            <button 
                                onClick={prevSlide} 
                                className="absolute left-4 md:left-6 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white/20 hover:bg-white/40 border border-white/20 flex items-center justify-center backdrop-blur-sm transition-opacity opacity-0 group-hover:opacity-100"
                            >
                                <ChevronLeft size={20} className="text-white" />
                            </button>
                            <button 
                                onClick={nextSlide} 
                                className="absolute right-4 md:right-6 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white/20 hover:bg-white/40 border border-white/20 flex items-center justify-center backdrop-blur-sm transition-opacity opacity-0 group-hover:opacity-100"
                            >
                            ...
                            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex space-x-2">
                                {slides.map((_, idx) => (
                                    <button 
                                        ...
                                        className={`h-1.5 rounded-full transition-all duration-300 shadow-sm ${idx === current ? 'bg-white w-8' : 'bg-white/50 w-2 hover:bg-white/80'}`}
                                    />
                                ))}
                            </div>
                        </>
...
export const HomePage: React.FC = () => {
  ...
  const renderHero = () => {
      if (config.heroMode === 'carousel' && config.heroSlides && config.heroSlides.length > 0) {
          return <HeroCarousel slides={config.heroSlides} />;
      }
      return (
