            ...
            <tbody className="divide-y">
              {categories.map(c => (
                <tr key={c.id}>
                  ...
                  <td className="px-6 py-4 flex space-x-2">
                    <button onClick={() => startEdit(c)} className="text-blue-600"><Edit size={18} /></button>
                    <button onClick={() => { if(confirm('Delete?')) deleteCategory(c.id); }} className="text-rose-600"><Trash size={18} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
