...
import { 
  ...
  FileText, Footprints, Palette, Code2, ArrowUp, ArrowDown, Move, RotateCcw, MonitorPlay, AlignLeft, AlignCenter, AlignRight, PanelTop, Check
} from 'lucide-react';
...
export const AdminProducts: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Convert comma-separated strings back to arrays if user edited them as strings
    // (Assuming simple binding for this demo)
    const processed = { ...editing };
    if (typeof processed.sizes === 'string') processed.sizes = (processed.sizes as string).split(',').map(s => s.trim()).filter(Boolean);
    if (typeof processed.colors === 'string') processed.colors = (processed.colors as string).split(',').map(s => s.trim()).filter(Boolean);
    if (typeof processed.images === 'string') processed.images = (processed.images as string).split(',').map(s => s.trim()).filter(Boolean);

    try {
      if (processed.id) {
        await updateProduct(processed as Product);
      } else {
        await addProduct(processed as Product);
      }
      setEditing(null);
    } catch (err) {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && editing) {
      ...
      reader.onloadend = () => {
        setEditing({ ...editing, images: [reader.result as string, ...(editing.images || []).slice(1)] });
      };
    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        <Button onClick={() => setEditing({ 
            ...
            images: [], sizes: [], colors: [], stock: 0, newArrival: false 
          })}>
            ...
            {products.map(p => (
              <tr key={p.id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-6 py-4 flex space-x-2">
                  <button onClick={() => setEditing(p)} className="p-2 hover:bg-gray-200 rounded text-blue-600"><Edit size={16}/></button>
                  <button onClick={() => handleDelete(p.id)} className="p-2 hover:bg-gray-200 rounded text-rose-600"><Trash size={16}/></button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setEditing(null)}><X size={24} /></button>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                 <Input label="Price ($)" type="number" value={editing.price} onChange={e => setEditing({...editing, price: Number(e.target.value)})} required />
                 <Input label="Discount Price ($)" type="number" value={editing.discountPrice || ''} onChange={e => setEditing({...editing, discountPrice: Number(e.target.value)})} />
                 <Input label="Stock" type="number" value={editing.stock} onChange={e => setEditing({...editing, stock: Number(e.target.value)})} />
              </div>

              <div>
                <label className="block text-sm font-medium mb-1">Description</label>
                <textarea 
                  className="w-full border p-2 text-sm h-24 rounded"
              ...
              <div className="grid grid-cols-2 gap-4">
                 <Input label="Sizes (comma separated)" value={Array.isArray(editing.sizes) ? editing.sizes.join(', ') : editing.sizes} onChange={e => setEditing({...editing, sizes: e.target.value})} />
                 <Input label="Colors (comma separated)" value={Array.isArray(editing.colors) ? editing.colors.join(', ') : editing.colors} onChange={e => setEditing({...editing, colors: e.target.value})} />
              </div>
              ...
              <div>
                ...
                <div className="flex items-center space-x-4">
                   {editing.images && editing.images[0] && (
                     <img src={editing.images[0]} className="w-16 h-16 object-cover rounded border" alt="Preview" />
                   )}
                   <input type="file" accept="image/*" onChange={handleImageUpload} className="text-sm" />
                </div>
                <Input 
                   label="Or Image URL" 
                   value={editing.images && editing.images[0]} 
              ...
              <div className="flex space-x-6 pt-2">
                 <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" checked={editing.newArrival} onChange={e => setEditing({...editing, newArrival: e.target.checked})} />
              ...
              <div className="flex justify-end pt-4 border-t">
                 <Button type="button" variant="outline" className="mr-3" onClick={() => setEditing(null)}>Cancel</Button>
...
export const AdminCategories: React.FC = () => {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    await addCategory(newCat as Category); // ID will be generated
    setIsAdding(false);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        <Button onClick={() => setIsAdding(true)}><Plus size={16} className="mr-2"/> Add Category</Button>
      </div>
        ...
        {categories.map(c => (
          <div key={c.id} className="bg-white rounded shadow-sm overflow-hidden group relative">
             ...
             <div className="p-4 flex justify-between items-center">
               ...
               <button onClick={() => deleteCategory(c.id)} className="text-gray-400 hover:text-rose-500"><Trash size={18}/></button>
             </div>
          </div>
        ))}
      </div>
           ...
           <div className="bg-white rounded p-6 w-full max-w-md">
              ...
              <form onSubmit={handleAdd} className="space-y-4">
                 ...
                 <div>
                    ...
                    <input type="file" onChange={handleImageUpload} className="text-sm mb-2" accept="image/*" />
                    <Input label="Or URL" value={newCat.image} onChange={e => setNewCat({...newCat, image: e.target.value})} />
                 </div>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        <Button onClick={() => setIsAdding(true)}><Plus size={16} className="mr-2"/> Add User</Button>
      </div>
             ...
             {users.map(u => (
               <tr key={u.id} className="border-b">
                 ...
                 <td className="px-6 py-4 capitalize">
                   <span className={`px-2 py-1 rounded text-xs ${u.role === 'admin' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100'}`}>{u.role}</span>
                 </td>
                 <td className="px-6 py-4">
                   {u.role !== 'admin' && (
